...
const PERIODOS = [
  { label: '15 días', days: 15,  meses: 1  },
  { label: '1 mes',   days: 30,  meses: 1  },
  { label: '3 meses', days: 90,  meses: 3  },
  { label: '6 meses', days: 180, meses: 6  },
  { label: '12 meses',days: 365, meses: 12 },
] as const;
...
export const Inicio: React.FC = () => {
  ...
  // ── Cobros en el período (payments pendientes) ──────────────
  const cobrosEnPeriodo = useMemo(() =>
    payments
      .filter(p => (p.status === 'pending' || p.status === 'overdue') && p.date >= hoyStr && p.date <= endDateStr)
      .sort((a, b) => a.date.localeCompare(b.date)),
    [payments, hoyStr, endDateStr]
  );

  // ── Gastos comprometidos en el período (no recurring) ───────
  ...
  const totalCobros      = cobrosEnPeriodo.reduce((s, p) => s + p.amount, 0);
  const totalGasFijos    = totalRecurring * periodo.meses;
  const totalCuotas      = debts.filter(d => d.activa).reduce((s, d) => s + d.cuotaMinima, 0) * periodo.meses;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingBottom: '5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div className="card" style={{ padding: '1.25rem', minHeight: 'auto' }}>
            ...
            <div style={{ fontSize: '2rem', fontWeight: 800, color: kpis.runway >= 3 ? '#10b981' : kpis.runway >= 1 ? '#f59e0b' : '#ef4444', marginTop: '0.2rem' }}>
              ...
              <span style={{ fontSize: '0.8rem', fontWeight: 500, color: '#52525b' }}> m</span>
            </div>
          </div>
          <div className="card" style={{ padding: '1.25rem', minHeight: 'auto' }}>
      ...
      {/* ── PANEL PRINCIPAL: Cobros + Proyección ────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>

        {/* Cobros y gastos en el período */}
        <div className="card" style={{ padding: '1.25rem' }}>
          <h3 style={{ color: '#fff', fontWeight: 700, marginBottom: '1rem', fontSize: '0.9rem' }}>
            📅 Próximos {periodo.label}
          </h3>

          {cobrosEnPeriodo.length > 0 && (
            <div style={{ marginBottom: '0.875rem' }}>
              <div style={{ fontSize: '0.65rem', color: '#10b981', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <TrendingUp size={10} />Cobros esperados ({cobrosEnPeriodo.length})
              </div>
              <div style={{ maxHeight: '220px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0' }}>
                {cobrosEnPeriodo.map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', borderBottom: '1px solid #1a1a1a' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '0.65rem', color: '#52525b' }}>{fmtDate(p.date)}</div>
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ fontSize: '0.78rem', color: '#10b981', fontWeight: 700, marginTop: '0.5rem', textAlign: 'right' }}>
                Total: {fmtK(totalCobros)}
              </div>
            </div>
          )}
          ...
          {gastosEnPeriodo.length > 0 && (
            <div>
              <div style={{ fontSize: '0.65rem', color: '#ef4444', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <TrendingDown size={10} />Gastos comprometidos ({gastosEnPeriodo.length})
              </div>
              <div style={{ maxHeight: '160px', overflowY: 'auto' }}>
                {gastosEnPeriodo.map(m => (
                  <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', borderBottom: '1px solid #1a1a1a' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: '0.65rem', color: '#52525b' }}>{fmtDate(m.fecha)}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {cobrosEnPeriodo.length === 0 && gastosEnPeriodo.length === 0 && (
            <div style={{ color: '#52525b', fontSize: '0.85rem', textAlign: 'center', padding: '2rem 0' }}>
              Sin movimientos en los próximos {periodo.label} 🎉
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ padding: '1.25rem', borderTop: `3px solid ${balanceProyectado >= 0 ? '#10b98155' : '#ef444433'}` }}>
          ...
          {/* Balance grande */}
          <div style={{ marginBottom: '1rem', padding: '1rem', background: '#0d0d0d', borderRadius: '10px' }}>
            ...
            <div style={{ fontSize: '2rem', fontWeight: 900, color: balanceProyectado >= 0 ? '#10b981' : '#ef4444', lineHeight: 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.45rem 0.75rem', background: 'rgba(239,68,68,0.07)', borderRadius: '8px' }}>
              <span style={{ fontSize: '0.82rem', color: '#a0aec0' }}>Gastos fijos ({periodo.meses} m)</span>
            ...
            {totalCuotas > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.45rem 0.75rem', background: 'rgba(249,115,22,0.07)', borderRadius: '8px' }}>
                <span style={{ fontSize: '0.82rem', color: '#a0aec0' }}>Cuotas deuda ({periodo.meses} m)</span>
            ...
            {totalComprometido > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.45rem 0.75rem', background: 'rgba(245,158,11,0.07)', borderRadius: '8px' }}>
                <span style={{ fontSize: '0.82rem', color: '#a0aec0' }}>Comprometidos</span>
          ...
          {totalCobros === 0 && (
            <div style={{ fontSize: '0.72rem', color: '#52525b', background: '#111', borderRadius: '8px', padding: '0.5rem 0.75rem' }}>
              💡 Sin cobros registrados para este período. Registra los pagos esperados en Proyectos.
            </div>
          )}
        </div>
      </div>
